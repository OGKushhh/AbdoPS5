...
static KYTY_SYSV_ABI int ShareCaptureScreenshot(const void* param, int32_t* req_id) {
        ...
        // Kyty-022: Trigger a screenshot capture on the next frame present.
        if (auto* screenshot = Libs::Graphics::GetScreenshot()) {
                screenshot->Request();
                if (req_id != nullptr) {
                        *req_id = 1; // fake request ID — the capture is async
                }
