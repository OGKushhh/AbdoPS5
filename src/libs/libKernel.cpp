...
static KYTY_SYSV_ABI KernelModule KernelLoadStartModule(const char* module_file_name, size_t args,
        ...
        if (!Common::File::IsFileExisting(module_path)) {
                ...
                // If the module file doesn't exist on disk (no firmware dump),
                // check if we have an HLE library that can serve it.
                auto hle_name = Libs::HleFallback::GetHleLibraryName(module_file_name);
                if (!hle_name.empty()) {
                        Libs::HleFallback::RecordFallback(module_file_name);
                        // Return a fake module handle (0x1).
                        // The HLE library's functions are already registered
                        // via LIB_DEFINE, so game imports will resolve to them.
                        if (res != nullptr) {
                                *res = OK;
                        }
                        return static_cast<KernelModule>(0x1);
                }
