...
bool RenderContext::HandleFault(PageFaultAccess access, uint64_t fault_vaddr) noexcept {
        // Kyty-016: When the IOMMU is enabled (rare — the boot loader
        // disables it), the GPU's device address (IOVA) should be
        // translated to a physical address via Iommu::Translate()
        // before looking it up in the guest address space.
        //
        // Currently the GPU uses guest VAs directly (no IOVA translation).
        // This is correct for the common case (IOMMU disabled). When we
        // encounter a game that actually uses IOMMU translation, we'll
        // add: if (auto* iommu = GetIommu(); iommu && iommu->IsEnabled())
        //     fault_vaddr = iommu->Translate(fault_vaddr);
        // here, before the IsMapped check.
        //
