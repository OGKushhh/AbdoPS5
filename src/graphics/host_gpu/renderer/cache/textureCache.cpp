...
void TextureCache::RefreshImage(ImageId id) {
	...
	TrackImage(id);
	// Path A.2: if the page tracker says nothing has been GPU-written
	// to this image's guest range, skip the expensive hash computation.
...
void TextureCache::MarkGpuWritten(ImageId id) {
	...
	// Path A.1: notify the alias registry so it can bump the per-image
	// generation counter and mark overlapping readers as maybe-dirty.
	...
	// Path A.2: bump per-page generation counters for the writer's
	// guest address range so readers can cheaply detect staleness.
