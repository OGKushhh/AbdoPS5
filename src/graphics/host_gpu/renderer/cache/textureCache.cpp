...
void TextureCache::RefreshImage(ImageId id) {
        ...
        TrackImage(id);
        // Kyty-037: if the page tracker says nothing has been GPU-written
        // to this image's guest range since the last refresh, AND the
        // image isn't already maybe-dirty, skip the expensive hash
        // computation entirely. This is a cheap O(pages) check that
        // avoids HashGuestEdges() for the common case of a texture that
        // hasn't changed since the last frame.
        if (!image.IsMaybeCpuDirty() && !image.IsDefinitelyCpuDirty() &&
            !image.IsBufferModified()) {
                const auto gen = m_page_tracker.MaxGeneration(
                    image.info.data.address, image.info.data.size);
                if (gen == GpuPageTracker::kInitialGeneration) {
                        // No GPU writes have ever touched this range — nothing to refresh.
                        return;
                }
        }
...
void TextureCache::MarkGpuWritten(ImageId id) {
        ...
        // generation counter and report overlapping readers. Mark each
        // overlapping reader as maybe-dirty so the next read triggers a
        // refresh instead of returning stale data.
        const auto overlaps = m_alias_registry.NotifyGpuWrite(id, m_scheduler.CurrentTick());
        for (const auto alias_id : overlaps) {
                auto* alias_image = m_slot_images.try_get(alias_id);
                if (alias_image != nullptr && alias_image->registered && !alias_image->depth_id) {
                        MarkAsMaybeDirty(alias_id, *alias_image);
                }
        }
