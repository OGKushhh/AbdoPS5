...
void PipelineCache::InitializeDriverCache() {
        ...
        // Kyty-015: Initialize the shader skip list for this game.
        // Loads data/shader_skips/<title_id>.json if it exists.
        Libs::Graphics::ShaderSkipList::Init(title_id);
...
// them into the main cache. Called at the start of GetGraphicsPipeline
// and GetComputePipeline (on the render thread).
void PipelineCache::PollAsyncResults() {
        Common::LockGuard lock(m_async_mutex);

        // Check graphics pipelines
        for (auto it = m_async_gfx_pending.begin(); it != m_async_gfx_pending.end();) {
                auto& pending = **it;
                if (pending.done.load(std::memory_order_acquire)) {
                        if (!pending.failed.load() && pending.pipeline != nullptr) {
                                Common::LockGuard main_lock(m_mutex);
                                auto [iter, inserted] = m_graphics_pipelines.emplace(
                                    std::move(pending.key), std::move(pending.pipeline));
                                if (inserted) {
                                        PipelineCacheLog("Vulkan async pipeline: graphics compiled (merged into cache)");
                                }
                        }
        ...
        for (auto it = m_async_compute_pending.begin(); it != m_async_compute_pending.end();) {
                ...
                if (pending.done.load(std::memory_order_acquire)) {
                        if (!pending.failed.load() && pending.pipeline != nullptr) {
                                Common::LockGuard main_lock(m_mutex);
                                auto [iter, inserted] = m_compute_pipelines.emplace(
                                    pending.key, std::move(pending.pipeline));
                                if (inserted) {
                                        PipelineCacheLog("Vulkan async pipeline: compute compiled (merged into cache)");
                                }
                        }
...
PipelineCache::Pipeline& PipelineCache::GetGraphicsPipeline(
    ...
    bool primitive_restart_enable, const GraphicsPrograms& programs) {
        // Kyty-032: Poll for finished async compilations before doing anything.
        PollAsyncResults();
        ...
        // Kyty-032: Check if this pipeline is already being compiled async.
        // If so, return a placeholder (the caller will skip the draw this frame).
        {
                Common::LockGuard async_lock(m_async_mutex);
                for (const auto& pending : m_async_gfx_pending) {
                        if (pending->key == key) {
                                // Still compiling — return a static placeholder.
                                // The draw will be skipped because pipeline.pipeline == nullptr.
                                static Pipeline s_placeholder{};
        ...
        // Kyty-032: Launch pipeline creation on a background thread.
        // Copy all the needed data so the background thread doesn't
        // reference stack-local variables.
        auto pending = std::make_unique<AsyncPendingGfx>();
        pending->key = key;

        // Capture copies of all data CreatePipelineInternal needs.
        // The shader programs (vertex_info, ps_input_info, programs) contain
        // vk::ShaderModule handles which are safe to use from any thread
        // (Vulkan objects are externally synchronized, and we hold m_mutex
        // on the render thread while the background thread compiles).
        auto* pending_ptr = pending.get();
        auto& graphics_ref = m_graphics;
        ...
        std::thread([pending_ptr, &graphics_ref, driver_cache,
                     rendering_copy, vertex_input_copy, static_params_copy,
                     vertex_info_vec = std::move(vertex_info_copy),
        ...
        // Return a placeholder immediately — the draw will be skipped this frame.
        // Next frame, PollAsyncResults() will merge the compiled pipeline
        // into m_graphics_pipelines, and the cache lookup will find it.
        static Pipeline s_placeholder{};
        return s_placeholder;
}
...
PipelineCache::GetComputePipeline(const ShaderComputeInputInfo& input_info,
        ...
        // Kyty-032: Poll for finished async compilations.
        PollAsyncResults();
        ...
        // Kyty-032: Launch compute pipeline creation on a background thread.
