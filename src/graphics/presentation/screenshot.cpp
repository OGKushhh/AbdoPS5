...
// Minimal PNG encoder for RGBA8 data using uncompressed deflate blocks.
// PNG format: signature + IHDR + IDAT + IEND chunks.
// The IDAT data uses zlib format with uncompressed (stored) deflate blocks
// (no compression — just wraps the raw data in zlib/deflate framing).
// This avoids the zlib library dependency entirely.
namespace {
...
uint32_t crc32_update(uint32_t crc, const uint8_t* data, size_t len) {
    ...
    static constexpr uint32_t TABLE[256] = {
        ...
        0xF00F9344, 0x8708A3D2, 0x1E01F268, 0x6906C2FE, 0xF7625757, 0x806567CB,
...
// Build a zlib stream with uncompressed (stored) deflate blocks.
// Format: zlib header (2 bytes) + N stored blocks + Adler-32 (4 bytes).
// Each stored block: 1 byte (BFINAL + BTYPE=00) + 2 bytes LEN + 2 bytes NLEN + data.
// Max stored block size = 65535 bytes.
void build_zlib_stored(const uint8_t* data, size_t len, std::vector<uint8_t>& out) {
    ...
    size_t offset = 0;
    while (offset < len) {
        const size_t block_len = std::min(len - offset, static_cast<size_t>(65535));
        const bool is_final = (offset + block_len >= len);

        out.push_back(is_final ? 0x01 : 0x00); // BFINAL + BTYPE=00 (stored)
        out.push_back(block_len & 0xFF);        // LEN low
        out.push_back((block_len >> 8) & 0xFF);  // LEN high
        const uint16_t nlen = ~static_cast<uint16_t>(block_len);
        out.push_back(nlen & 0xFF);             // NLEN low
        out.push_back((nlen >> 8) & 0xFF);       // NLEN high
        out.insert(out.end(), data + offset, data + offset + block_len);
...
std::string Screenshot::SaveToPng(uint32_t width, uint32_t height,
    ...
    // Build raw image data with PNG filter bytes (one per row, filter=None).
    ...
    // Compress using zlib stored blocks (no actual compression — avoids
    // the zlib library dependency entirely).
    std::vector<uint8_t> compressed;
    build_zlib_stored(raw_data.data(), raw_data.size(), compressed);
