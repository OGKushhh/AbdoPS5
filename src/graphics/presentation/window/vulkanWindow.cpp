...
static void VulkanFindPhysicalDevice(vk::Instance instance, vk::SurfaceKHR surface,
                                     ...
                                     bool& out_has_fragment_barycentric,
                                     bool& out_has_color_write_enable,
                                     bool& out_has_depth_clip_enable) {
        ...
        bool                best_has_barycentric   = false;
        bool                best_has_color_write    = false;
        bool                best_has_depth_clip    = false;
        ...
        for (const auto& device: devices) {
                ...
                // Kyty-vulkan-relax: when --vulkan-relax-requirements is set,
                // missing colorWriteEnable / depthClipEnable / fragmentShaderBarycentric
                // become warnings instead of device-rejection errors. Lets the
                // emulator boot on GPUs that predate these extensions (e.g. Pascal
                // GTX 10-series lacks fragmentShaderBarycentric).
                const bool relax = Config::VulkanRelaxRequirements();

                if (color_write_ext.colorWriteEnable != VK_TRUE) {
                        LOGF("colorWriteEnable is not supported%s\n", relax ? " (relaxed)" : "");
#if !defined(__APPLE__)
                        if (!relax) {
                                skip_device = true;
                        }
                ...
                if (depth_clip_enable.depthClipEnable != VK_TRUE) {
                        LOGF("depthClipEnable is not supported%s\n", relax ? " (relaxed)" : "");
#if !defined(__APPLE__)
                        if (!relax) {
                                skip_device = true;
                        }
                ...
                if (fragment_barycentric.fragmentShaderBarycentric != VK_TRUE) {
                        LOGF("fragmentShaderBarycentric is not supported%s\n", relax ? " (relaxed)" : "");
                        if (!relax) {
                                skip_device = true;
                        }
                }
                ...
                if (best_device == nullptr ||
                    device_properties.deviceType == vk::PhysicalDeviceType::eDiscreteGpu) {
                        ...
                        best_has_barycentric    = (fragment_barycentric.fragmentShaderBarycentric == VK_TRUE);
                        best_has_color_write     = (color_write_ext.colorWriteEnable == VK_TRUE);
                        best_has_depth_clip      = (depth_clip_enable.depthClipEnable == VK_TRUE);
                }
        }
        ...
        out_has_fragment_barycentric = best_has_barycentric;
        out_has_color_write_enable   = best_has_color_write;
        out_has_depth_clip_enable    = best_has_depth_clip;
}
...
static vk::Device VulkanCreateDevice(GraphicContext& graphics,
        ...
        vk::PhysicalDeviceColorWriteEnableFeaturesEXT color_write_ext {};
        // Kyty-vulkan-relax: only request colorWriteEnable if the device
        // actually supports it. Without this check, vkCreateDevice would
        // fail with VK_ERROR_FEATURE_NOT_PRESENT on older GPUs (e.g. Pascal
        // GTX 10-series without recent driver updates).
        color_write_ext.colorWriteEnable =
            graphics.color_write_enable_enabled ? VK_TRUE : VK_FALSE;

        vk::PhysicalDeviceDepthClipEnableFeaturesEXT depth_clip_enable {};
        depth_clip_enable.pNext = &color_write_ext;
        depth_clip_enable.depthClipEnable =
            graphics.depth_clip_enable_enabled ? VK_TRUE : VK_FALSE;
        ...
        // Kyty-vulkan-relax: only request fragmentShaderBarycentric if the
        // device actually supports it. Pascal (GTX 10-series) predates this
        // extension entirely — requesting it would cause vkCreateDevice to
        // fail with VK_ERROR_FEATURE_NOT_PRESENT.
...
void WindowContext::CreateVulkan() {
        ...
        VulkanFindPhysicalDevice(graphic_ctx.instance, surface, device_extensions, surface_capabilities,
                                 ...
                                 graphic_ctx.color_write_enable_enabled,
                                 graphic_ctx.depth_clip_enable_enabled);
