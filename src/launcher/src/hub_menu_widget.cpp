...
void HubMenuWidget::SetupUI() {
        ...
        // Create the shared opacity effect exactly once. Reused by both
        // AnimateIn() and AnimateOut() — see the comment in the header.
        m_opacityEffect = new QGraphicsOpacityEffect(this);
        m_opacityEffect->setOpacity(1.0); // fully opaque by default
        setGraphicsEffect(m_opacityEffect);
}

void HubMenuWidget::closeEvent(QCloseEvent* event) {
        // Make sure the parent's m_cinema_mode flag is cleared when the
        // user closes via the title-bar X button (not just via Esc).
        emit backToMainView();
...
void HubMenuWidget::AnimateIn() {
        ...
        // Stop any running fade before starting a new one — otherwise two
        // QPropertyAnimations would race against the same opacity property.
        if (m_fadeAnim) {
                m_fadeAnim->stop();
        }
...
void HubMenuWidget::AnimateOut() {
        ...
        connect(m_fadeAnim, &QPropertyAnimation::finished, this, &HubMenuWidget::hide);
        m_fadeAnim->start(QAbstractAnimation::DeleteWhenStopped);
}
...
void HubMenuWidget::keyPressEvent(QKeyEvent* event) {
        switch (event->key()) {
                ...
                case Qt::Key_Enter:
                        ...
                        break;
                case Qt::Key_Escape:
                        // close() fires closeEvent, which is the single emitter
                        // for backToMainView() — see the comment in closeEvent.
                        close();
                        break;
                default:
