...
private:
        ...
        // Single shared opacity effect — created once in SetupUI() and
        // reused by both AnimateIn() and AnimateOut(). Previously each
        // call created a fresh QGraphicsOpacityEffect, deleting the prior
        // one while a QPropertyAnimation was still referencing it →
        // use-after-free → segfault on rapid toggles.
